...
export function* addProgrammer(action) {
  try {
    ...
    const isDuplicated = yield select(state =>
      state.programmers.data.find(programmer => programmer.id == data.id)
    );

    if (isDuplicated) {
      debugger;
      yield put(
        ProgrammerActions.addProgrammerFailure(
    ...
    debugger;
    yield put(
      ProgrammerActions.addProgrammerFailure("Erro ao adicionar usuário")
    );
  }
}
